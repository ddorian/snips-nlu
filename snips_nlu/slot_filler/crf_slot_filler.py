...
class CRFSlotFiller(SlotFiller):
    ...
    def persist(self, path):
        ...
        if self.crf_model is not None:
            crf_model_file = path / Path(self.crf_model.modelfile.name).name
            shutil.copy(self.crf_model.modelfile.name, str(crf_model_file))

        model = {
            "unit_name": self.unit_name,
            "language_code": self.language,
            "intent": self.intent,
            "crf_model_file": str(crf_model_file.name),
